###########################################################################
###########################################################################
######## INTENSHIP PROJECT - proteins (ANOVA) code (FINAL VERSION) ########
###########################################################################
###########################################################################
...
foreach(group_col = group_cols, .packages = c("dplyr", "glmnet", "caret", "parallel", "doParallel"), .export = "perform_case_cohort_anova") %dopar% {
  ...
  # Step 1: ANOVA
  cat("\n--- anova ---\n")
  anova_results <- sapply(feature_data, function(x) {
    perform_case_cohort_anova(x, risk_groups, weights = weights)
  })
  ...
  top_features <- anova_df %>%
    dplyr::filter(p_value < 0.05) %>%
    dplyr::pull(feature)
  
  if (length(top_features) == 0) return(NULL)
  
  # Step 2: Prepare data
  cat("\n--- data prep ---\n")
  X_top <- as.matrix(feature_data[, ..top_features])
